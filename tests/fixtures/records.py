        ...
        def _deep_merge(base: dict, overrides: dict) -> dict:
            ...
            for key, override_value in overrides.items():
                if (
                    ...
                    base[key] = _deep_merge(base[key], override_value)
                else:
                    base[key] = override_value
            return base

        # Start from a complete minimal metadata, then merge any user-provided parts
        input_metadata = deepcopy(record_metadata().metadata_in)
        if metadata:
            input_metadata = _deep_merge(input_metadata, deepcopy(metadata))
